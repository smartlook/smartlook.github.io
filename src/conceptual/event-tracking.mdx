...
In the case that you want to measure the duration of some time-sensitive or long-running actions in the app, Smartlook offers timed events. Nothing is tracked upon starting such an event, the app just remembers the event reference and uses it once the event is finished or cancelled. For details, see the
