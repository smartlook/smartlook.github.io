...
The support for individual sensitive data protecting methods depends heavily on the technology used for creating the particular user intefrace. 
...
<TextBlock kind="note" visibleOn="ios">{`
On iOS, it is also possible to blacklist or whitelist all instances of a **\`UIView\` subclass** or all \`UIView\` or all UIView subclasses that conform to a **protocol**. More on the topic in <Link to="/docs/sdk/api-reference#sensitive-data-hiding">API reference</Link>.
`}</TextBlock>
