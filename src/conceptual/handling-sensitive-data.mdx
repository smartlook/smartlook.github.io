...
There can be scenarios when direct recording of the screen is not appropriate or necessary to understand the user behaviour. In such  cases, Smartlook can be instructed to use (permanently or just temporarily) one of its wireframe recording modes.
...
The default recording mode for an app is set in [the dashboard](https://app.smartlook.com/settings/projects). 

When debugging or experimenting to find the most suitable one for a particular appRecording mode, the default rendering mode can be also set within the setup options. In production releases, this option should not be used.
