...
In order not to record a particular view, like here in an example with an image view that shows a portrait image of the user, use a variable that holds a reference to the view with one of the following approaches to blacklist it, e.g.,
`}</TextBlock>
...
<TextBlock>{`
More on this topic in <Link to="/docs/sdk/api-reference?platform=ios#sensitive-data-hiding">API reference</Link>.
`}</TextBlock>
...
<TextBlock>{`
More about rendering modes can be found <Link to="/docs/sdk/api-reference#event-tracking-modes">in the API reference.</Link>
`}</TextBlock>
