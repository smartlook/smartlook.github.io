...
Apart from video or wireframe data, SDK can also send additional data needed for Interactive player. Currently this BETA functionality is accessible via builder. Read more about Interactive player feature [in conceptual documentation](/docs/sdk/conceptual/interactive-player).:
`}</TextBlock>
...
<TextBlock kind="note" visibleOn="android">{`
\`WEBGL_ONLY\` mode does not record any screen data, similar to \`NO_RENDERING\`, however you will be able to inspect app's structure in our Interactive player. \`WEBGL_HYBRID\` mode sends video or wireframe data and WebGL related data as well. [read more](#no-rendering).
`}</TextBlock>
