...
<TextBlock visibleOn="android">{`
## WebGL Player - BETA
Apart from video or wireframe data, SDK can also send additional data needed for WebGL player. Currently this BETA functionality is accessible via builder. Read more about WebGL feature [in conceptual documentation](/conceptual/handling-sensitive-data#recording-modes-handling-sensitive-data-at-the-whole-screen-level).:
`}</TextBlock>
...
<TextBlock visibleOn="android">{`
WebGL modes available are following:
`}</TextBlock>
...
<TextBlock kind="note" visibleOn="android">{`
\`WEBGL_ONLY\` mode does not record any screen data, similar to \`NO_RENDERING\`, however you will be able to inspect app's struccture in our WebGL player. \`WEBGL_HYBRID\` mode sends video or wireframe data and WebGL related data as well. [read more](#no-rendering).
`}</TextBlock>
