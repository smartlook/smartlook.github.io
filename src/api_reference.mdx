...
When using **native** rendering mode, **SDK can record sensitive data in your application**. Recording can be configured with the methods listed below so sensitive data is not recorded, or **Wireframe** rendering mode can be set because it does not record any sensitive data by design.
`}</TextBlock>
...
Some application elements should not be recorded because of security (posibly GDPR) reasons.
...
<TextBlock visibleOn="react">{`
When an application has some parts displayed using \`WebView\` or \`WKWebView\` and theese parts should be recorded, then the \`WebView\` or \`WKWebView\` recording needs to be enabled by adding the given view reference to whitelist:
`}</TextBlock>
...
[Smartlook unregisterBlacklistedObject:WKWebView.class];
    ...
    react: {
      javascript: `
Smartlook.registerWhitelistedView(webViewRef);
    `,
    },
xamarin: {
      csharp: `
