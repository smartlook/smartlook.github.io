...
Smartlook attempts to hide selected sensitive UI element automatically. It can be also instructed to hide or show particular UI components. Alternativelly, using one of the **wireframe** rendering modes records screen in a schematic way, showing no user data. The last possibility is to stop screen capturing altogether by using **no rendering** mode.
`}</TextBlock>
...
On this platform, hidding selectivelly sensitive parts of the screen is not possible.\n
