...
<TextBlock kind="note" visibleOn="android">{`
\`WEBGL_ONLY\` mode does not record any screen data, similar to \`NO_RENDERING\`, however you wi’ll be able to inspect the app's structure in our interactive player. \`WEBGL_HYBRID\` mode sends video or wireframe data and WebGL-related data as well. [read more](#no-rendering).
`}</TextBlock>
