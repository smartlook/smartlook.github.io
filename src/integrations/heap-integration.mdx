...
Smartlook can be automatically integrated into Heap using:
`}</TextBlock>
...
\`enableIntegration\` integration must be called after Heap is intialized.
`}</TextBlock>

<TextBlock visibleOn="android">{`
Smartlook visitor URL will be added as a new property called \`Smartlook visitor dashboard URL\` into a Heap user.
`}</TextBlock>
