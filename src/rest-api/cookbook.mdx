...
2. There are 2 endpoints to delete visitors and their related events and sessions
    1. Delete only 1 visitor by `visitorId` (unique ID obtained by Smartlook) - this `visitorId` fill in to the path
    2. Delete more visitors by `userId` (`userId` is set by identify method in the SDK)